...
//网络时间处理，signale信号处理都在这里signal_handler
//signal_handler信号处理  lxc_cmd_handler网络读写处理

//??????????????? lxc-stop发送kill信号后，为什么这里要过很久才能受到该事件??????????????
//信号处理可能被父进程接受到也可能被容器子进程接收到，因为他们公用同一个epoll
...
//lxc_cmd_accept(lxc_cmd_handler)  signal_handler
static int lxc_poll(const char *name, struct lxc_handler *handler)
{
    ...
    //lxc_cmd_process发送相应signal信号给lxc-start容器进程，容器子进程会在signal_handler受到信号，然后做相应处理
...
*/ //非lxc.cgroup.devices.XX相关的操作，后面cgroup_setup_limits是devices相关配置
	...
	if (handler->ops->post_start(handler, handler->data)) //post_start  :post_start:1294 - '/sbin/init' started with pid '15910'
		goto out_abort;
	...
	lxc_sync_fini(handler); //关闭主从进程通信的pipe
		...
		struct lxc_operations* ops, void *data, const char *lxcpath)
{
	...
	err = lxc_poll(name, handler); //epoll loop载这里
	if (err) {
